    ...
    return (
      <Link href={`${pathname}/${lesson.id}`}>
        <div className="relative h-[240px] md:h-[300px]">
          {lesson.imageUrl && (
            <div className="relative h-[240px] md:h-[300px] w-[170px] md:w-[200px] rounded rounded-bl-3xl hover:opacity-90">
              <Image
                src={lesson.imageUrl}
                alt="job_imgge"
          ...
          <div className="absolute bottom-4 right-6 text-white font-semibold z-30 text-sm md:text-base">
            <p className="text-right">{lesson.name}</p>
            <div className="flex gap-4 justify-end">
              <span>피아노</span>
              <span>서울 전체</span>
              {/* <span>{lesson.majors}</span>
              <span>{lesson.provinces}</span> */}
            </div>
          </div>
        </div>
